...
2. 新文章必须引用现有 `assets/css/style.css`，位于 `articles/<分类>/` 的页面使用相对路径 `../../assets/css/style.css`。
...
6. 文章图片统一放在文章同名目录的 `images/` 中，例如 `articles/mcu/<slug>/images/`；附件放在同级 `downloads/`。可以先写本地占位路径，但不得引用 CSDN 外链图片。
7. 不改变既有 CSS、JS、布局或无关文章内容，除非用户明确要求。

## workflow

1. 先查看同分类现有文章和首页对应入口，确认相对路径、缓存版本与卡片结构。
2. 以 `templates/article-template.html` 为起点创建文章，再替换标题、摘要、正文和图片路径。
